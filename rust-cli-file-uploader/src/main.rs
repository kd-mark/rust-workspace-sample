...
use std::{env, fs::File, io::{self, Read}, path::Path};
...
async fn upload_file(client: &Client, url: &str, file_name: String, file_data: Vec<u8>) -> Result<(), reqwest::Error> {
...
async fn main() {
    ...
    match read_file(file_path).await {
        Ok((file_name, file_data)) => {
            ...
            if let Err(err) = upload_file(&client, url, file_name, file_data).await {
